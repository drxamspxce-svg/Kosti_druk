...
@private_router.message(
    CommandStart(),
    F.chat.type == "private",
    F.from_user.id == ADMIN_ID  # ← фильтр на уровне роутера
)
...
# ─── FSM: ОЖИДАНИЕ КОНТЕНТА РАССЫЛКИ ─────────────────────────────────────────
...
async def broadcast_receive(message: types.Message, state: FSMContext, bot: Bot):
    ...
    for chat_id in users:
        try:
            # Пересылаем сообщение как есть — сохраняет медиа, эмодзи, ссылки
...
async def create_new_list_prompt(message: types.Message):
    ...
    await message.answer(
        ...
        "Напиши /sendlist в группе чтобы вывести пустой шаблон."
    )
...
# ─── МАССОВЫЙ ИМПОРТ (всегда последний!) ─────────────────────────────────────
