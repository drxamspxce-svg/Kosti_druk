...
async def user_start(message: types.Message):
    # Админ обрабатывается в своём роутере
...
async def notify_player(bot: Bot, username: str, limit: int):
    """
    Вызывается из group.py когда наступает ход игрока.
    Ищет chat_id по username и шлёт уведомление в лс.
    """
    from database import get_chat_id_by_username
    chat_id = get_chat_id_by_username(username)

    if not chat_id:
        return  # Игрок не зарегистрирован — молча пропускаем
    ...
    try:
        ...
        # Игрок заблокировал бота — ничего не делаем
        pass
